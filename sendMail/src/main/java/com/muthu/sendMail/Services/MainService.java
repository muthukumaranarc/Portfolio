...
public class MainService {
    ...
    public boolean sendMailToVisitor(UserInput data) {
        ...
        String body = "<html>" +
                ...
                "</blockquote>" +
                "<p>It’s amazing to hear thoughts from professionals in <b>" + data.getRoleInIT() + "</b> with <b>"
                + data.getExperience() + " years</b> of experience.</p>" +
    ...
    public ResponseEntity<String> getStats() {
        try {
            ...
            String graphQLQuery = "{\"query\":\"query getUserProfile($username: String!) { matchedUser(username: $username) { profile { realName reputation } submitStats: submitStatsGlobal { acSubmissionNum { difficulty count submissions } } } }\",\"variables\":{\"username\":\""
                    + username + "\"}}";
            ...
            org.springframework.http.HttpEntity<String> entity = new org.springframework.http.HttpEntity<>(graphQLQuery,
                    headers);
    ...
    private String processLeetCodeResponse(String response) {
        try {
            ...
            com.fasterxml.jackson.databind.JsonNode submitStats = root.path("data").path("matchedUser")
                    .path("submitStats").path("acSubmissionNum");
            ...
            String result = String.format(
                    "{\"totalSolved\":%d,\"totalQuestions\":%d,\"easySolved\":%d,\"mediumSolved\":%d,\"hardSolved\":%d,\"totalEasy\":%d,\"totalMedium\":%d,\"totalHard\":%d}",
                    totalSolved, totalQuestions, easySolved, mediumSolved, hardSolved, totalEasy, totalMedium,
                    totalHard);
